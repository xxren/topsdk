<?php
/**
 * TOP API: taobao.item.vip.add request
 * 
 * @author auto create
 * @since 1.0, 2015.11.23
 */
class ItemVipAddRequest
{
	/** 
	 * 售后说明模板id
	 **/
	...
	/** 
	 * 定制工具Id如果支持定制市场，这个值没有设置，就默认为-1代表支持定制市场但是没有定制工具Id
	 **/
	...
	/** 
	 * 秒杀商品类型。暂时不能使用。打上秒杀标记的商品，用户只能下架并不能再上架，其他任何编辑或删除操作都不能进行。如果用户想取消秒杀标记，需要联系小二进行操作。如果秒杀结束需要自由编辑请联系活动负责人（小二）去掉秒杀标记。可选类型web_only(只能通过web网络秒杀)wap_only(只能通过wap网络秒杀)web_and_wap(既能通过web秒杀也能通过wap秒杀)
	 **/
	private $secondKill;
	...
	/** 
	 * 是否支持定制市场true代表支持，false或者为空代表不支持
	 **/
