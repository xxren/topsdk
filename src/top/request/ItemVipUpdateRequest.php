<?php
/**
 * TOP API: taobao.item.vip.update request
 * 
 * @author auto create
 * @since 1.0, 2016.04.25
 */
class ItemVipUpdateRequest
{
	/** 
	 * 售后说明模板id
	 **/
	...
	/** 
	 * 全球购商品采购地（地区/国家）,默认值只在全球购商品采购地（库存类型选择情况生效）国家地区目前有（美国, 香港, 日本, 英国, 新西兰, 德国, 韩国, 荷兰, 澳洲, 法国, 意大利, 台湾, 澳门, 加拿大, 瑞士, 西班牙, 泰国, 新加坡, 马来西亚, 菲律宾, 其他）
	 **/
	...
	/** 
	 * 属性值别名。如pid:vid:别名;pid1:vid1:别名1， pid:属性id vid:值id。总长度不超过800字节
	 **/
	...
	/** 
	 * 秒杀商品类型。暂时不可用。打上秒杀标记的商品，用户只能下架并不能再上架，其他任何编辑或删除操作都不能进行。如果用户想取消秒杀标记，需要联系小二进行操作。如果秒杀结束需要自由编辑请联系活动负责人（小二）去掉秒杀标记。可选类型 web_only(只能通过web网络秒杀) wap_only(只能通过wap网络秒杀) web_and_wap(既能通过web秒杀也能通过wap秒杀)
	 **/
	private $secondKill;
	
	/** 
	 * 商品卖点信息，最长150个字符。天猫卖家和集市卖家都可用。
	 **/
	private $sellPoint;
	...
	/** 
	 * 宝贝主图9s视频的ID值，传入数字类型，标示该视频文件在淘宝视频中心的数字ID号。
	 **/
	private $videoId;
	...
	/** 
	 * 无线宝贝描述功能。如果不用更新宝贝描述，传入null或不传。如果需要删除，请在emptyFields加入该字段
	 **/
