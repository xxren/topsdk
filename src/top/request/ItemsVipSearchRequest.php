<?php
/**
 * TOP API: taobao.items.vip.search request
 * 
 * @author auto create
 * @since 1.0, 2016.03.17
 */
class ItemsVipSearchRequest
{
	/** 
	...
	/** 
	 * 需返回的商品结构字段列表。可选值为Item中的以下字段：num_iid,title,nick,pic_url,cid,price,type,delist_time,post_fee;多个字段用“,”分隔。如：num_iid,title。新增字段score(卖家信用等级数),volume(30天成交量),location（卖家地址，可以分别获取location.city,location.state）,num_iid商品数字id；新增字段：新增字段：has_discount, num, is_prepay, promoted_service, ww_status, list_time
	 **/
	private $fields;
	...
	/** 
	 * 特别搜索的商品的类型（如：搜索给雅虎日本的商品，这个字段传入1002）
	 **/
	private $itemType;
	...
	/** 
	 * 排序方式。格式为column:asc/desc,column可选值为: price（价格）, delist_time（下架时间）, seller_credit（卖家信用）；默认是按照上架时间倒序。如按价格升序排列表示为：price:asc。新增排序字段：volume（30天成交量），新增排序字段：popularity(商品的人气值)
	 **/
	private $orderBy;
	...
	/** 
	 * 商品属性。可以搜到拥有和输入的属性一样的商品列表。字段格式为：pid1:vid1;pid2:vid2属性的pid调用taobao.itemprops.get取得，属性值的vid用taobao.itempropvalues.get取得vid。
	 **/
	private $props;
	...
	{
		return "taobao.items.vip.search";
	}
