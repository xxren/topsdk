...
class TopMultiClient
{
	...
	public function prepareExecute($request, $session = null,$bestUrl = null)
	{
		...
		try
		{
            ...
            $ret = curl_multi_add_handle(self::$mh, $resp);
            self::$closeCh[data_get($apiParams, 'num_iid')] = $resp;

            //\Log::debug('curl_multi_add_handle', [$ret, $apiParams]);
            return $resp;
        }
		catch (Exception $e)
		{
...
\Log::debug('== multiExec ==', [2, $active, $mrc]);

        while ($active && $mrc == CURLM_OK)
        {
            \Log::debug('== multiExec ==', [0]);
            ...
            \Log::debug('== multiExec ==', [01]);

            do {
                $mrc = curl_multi_exec(self::$mh, $active);

                \Log::debug('== multiExec ==', [02]);
            } while ($mrc == CURLM_CALL_MULTI_PERFORM);
        }

\Log::debug('== multiExec ==', [3]);

        $data = [];
        foreach (self::$closeCh as $k => $ch) {
            $data[$k] = curl_multi_getcontent($ch);
            \Log::debug('==cul_multi==', [$data[$k]]);
            curl_multi_remove_handle(self::$mh, $ch);//移除句柄
            curl_close($ch);//关闭会话
        }
\Log::debug('== multiExec ==', [4]);
